...
# ─── CONFIG ──────────────────────────────────────────────────────────────────
BOT_TOKEN      = "8752142136:AAFkn7iT0A9u9Oi0ZJLNi68Jm4im9Vz98Yw"
ADMIN_IDS      = [7626680188]
ADMIN_GROUP_ID = -1003779625313
LOG_CHANNEL_ID = -1003800214171
API_ID         = 30191201
...
async def category_detail(update, context):
    ...
    if stock == 0:
        await query.edit_message_text(
            f"📂 *{mesc(c['name'])}*\n━━━━━━━━━━━━━━━━━━━━\n❌ *Out of stock!*",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="browse_0")]]))
        return
    conn = get_db()
    ...
    buttons = []
    if upi_on:
        buttons.append([InlineKeyboardButton("💳 Buy with UPI", callback_data=f"pay_upi_{cat_id}")])
    if crypto_on:
        buttons.append([InlineKeyboardButton("🪙 Buy with Crypto (OxaPay)", callback_data=f"pay_crypto_{cat_id}")])
    buttons.append([InlineKeyboardButton(f"💰 Buy from Wallet  (₹{wallet:.2f})", callback_data=f"wallet_buy_{cat_id}")])
    buttons.append([InlineKeyboardButton("🔙 Back", callback_data="browse_0")])
...
async def wallet_buy(update, context):
    ...
    if not acc:
        ...
        await query.edit_message_text("❌ No accounts available right now.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data=f"cat_{cat_id}")]]))
        return
    now = now_ist().isoformat()
...
async def pay_upi(update, context):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("📸 I've Paid — Upload Screenshot", callback_data=f"buy_upload_{cat_id}")],
        [InlineKeyboardButton("🔙 Back", callback_data=f"cat_{cat_id}")],
    ])
...
async def buy_upload_prompt(update, context):
    ...
    await query.message.reply_text(
        ...
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data=f"cat_{cat_id}")]]))
    try:
        await query.message.delete()
    except Exception:
        pass
...
async def pay_crypto(update, context):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("💳 Pay Now (OxaPay)", url=invoice["payLink"])],
        [InlineKeyboardButton("🔄 Check Status", callback_data=f"chk_ord_{order_id}")],
        [InlineKeyboardButton("🔙 Back", callback_data=f"cat_{cat_id}")],
    ])
...
async def screenshot_handler(update, context):
    ...
    if context.user_data.get("awaiting_deposit_screenshot"):
        ...
        kb = InlineKeyboardMarkup([[
            InlineKeyboardButton(f"✅ Approve #{dep_id}", callback_data=f"approve_deposit_{dep_id}"),
            InlineKeyboardButton(f"❌ Reject #{dep_id}",  callback_data=f"reject_deposit_{dep_id}"),
        ]])
        try:
            await context.bot.send_photo(chat_id=ADMIN_GROUP_ID, photo=file_id,
                                         caption=admin_text, reply_markup=kb)
...
async def reveal_number(update, context):
    ...
    kb = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔒 Logout Bot Session", callback_data=f"logout_prompt_{acc['id']}")],
        [InlineKeyboardButton("📦 My Orders",          callback_data="my_orders_0")],
    ])
    await query.edit_message_text(text, parse_mode="Markdown", reply_markup=kb)
...
async def get_otp(update, context):
    ...
    if not acc["session_string"]:
        await query.edit_message_text("ℹ️ Bot session already logged out.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back",
                callback_data=f"reveal_{_get_order_for_acc(acc_id)}")]]))
        return
    ...
    kb = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔒 Logout Bot Session", callback_data=f"logout_prompt_{acc_id}")],
...
async def getotp_back(update, context):
    # FIX: query.data can't be set — directly call show reveal logic
    query    = update.callback_query
    await query.answer()
...
async def logout_prompt(update, context):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔒 Yes, Logout Bot Now", callback_data=f"logout_confirm_{acc_id}")],
        [InlineKeyboardButton("❌ Cancel", callback_data=f"reveal_{_get_order_for_acc(acc_id)}")],
    ])
...
async def logout_confirm(update, context):
    ...
    if not acc["session_string"]:
        await query.edit_message_text("ℹ️ Bot session already logged out.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📦 My Orders", callback_data="my_orders_0")]]))
        return
...
async def wallet(update, context):
    ...
    buttons = []
    if upi_on:
        buttons.append([InlineKeyboardButton("➕ Deposit via UPI",    callback_data="deposit_upi")])
    if crypto_on:
        buttons.append([InlineKeyboardButton("🪙 Deposit via Crypto", callback_data="deposit_crypto")])
    buttons += [
...
async def text_handler(update, context):
    ...
    if context.user_data.get("awaiting_dep_amount"):
        ...
        if method == "upi":
            ...
            kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("💳 Pay Now (OxaPay)", url=invoice["payLink"])],
                [InlineKeyboardButton("🔄 Check Status", callback_data=f"chk_dep_{dep_id}")],
            ])
    ...
    if context.user_data.get("new_cat_step") == "phone":
        ...
        context.user_data["new_cat_step"]  = "session"
        await update.message.reply_text("✅ Phone saved!\n\n🔑 Send *session string*:", parse_mode="Markdown")
        return

    # ── Admin: session ──
    if context.user_data.get("new_cat_step") == "session":
        context.user_data["current_session"] = update.message.text.strip()
        context.user_data["new_cat_step"]     = "twofa"
        await update.message.reply_text(
            "✅ Session saved!\n\n🔐 Send *2FA password* or skip:",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏭ Skip 2FA", callback_data="skip_2fa_stock")]]))
        return

    # ── Admin: 2FA ──
    if context.user_data.get("new_cat_step") == "twofa":
        context.user_data["current_2fa"] = update.message.text.strip()
        await _save_account_and_continue(update, context)
        return

    # ── Admin: price (INR only — auto USDT) ──
    if context.user_data.get("new_cat_step") == "price":
        try:
            ...
            return
        usd      = inr_to_usd(inr)
        cat_id   = context.user_data.pop("new_cat_id", None)
        cat_name = context.user_data.pop("new_cat_name", "")
        context.user_data.pop("new_cat_step",     None)
        context.user_data.pop("new_cat_quantity", None)
        context.user_data.pop("new_cat_added",    None)
        conn = get_db()
        if cat_id:
            conn.execute("UPDATE stock_categories SET price_inr=?,price_usd=? WHERE id=?", (inr, usd, cat_id))
            conn.commit()
            stock = conn.execute("SELECT COUNT(*) as c FROM accounts WHERE category_id=? AND is_sold=0",
                                 (cat_id,)).fetchone()["c"]
        conn.close()
        rate = get_usdt_rate()
        await update.message.reply_text(
            ...
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"📂 Category: {mesc(cat_name)}\n"
            f"💰 Price: ₹{inr:.0f} INR → ${usd:.2f} USDT\n"
            f"📊 Rate: 1 USDT = ₹{rate:.0f}\n"
            f"📦 Stock: {stock} numbers\n"
            f"━━━━━━━━━━━━━━━━━━━━",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("➕ Add More", callback_data="add_stock_start")],
...
async def _save_account_and_continue(update, context):
    """FIX: Works for both message updates and callback query updates."""
    cat_id  = context.user_data["new_cat_id"]
    phone   = context.user_data.pop("current_phone")
    session = context.user_data.pop("current_session")
    ...
    # Get the message object — works for both message and callback_query updates
    msg = update.message or (update.callback_query.message if update.callback_query else None)
    if not msg:
        return
...
async def my_orders(update, context):
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("◀️", callback_data=f"my_orders_{page-1}"))
        nav.append(InlineKeyboardButton(f"{page+1}/{pages}", callback_data="noop"))
    if page < pages - 1:
        nav.append(InlineKeyboardButton("▶️", callback_data=f"my_orders_{page+1}"))
    if nav:
...
async def dep_hist(update, context):
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("◀️", callback_data=f"dep_hist_{page-1}"))
    if page < pages - 1:
        nav.append(InlineKeyboardButton("▶️", callback_data=f"dep_hist_{page+1}"))
...
async def help_cb(update, context):
    ...
    await query.edit_message_text(
        ...
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("💬 Support", url="https://t.me/PRIME_RAI07")],
            [InlineKeyboardButton("🔙 Main Menu", callback_data="main_menu")],
        ]))
...
async def admin_stock(update, context):
    ...
    conn.close()
    buttons = [[InlineKeyboardButton("➕ Add New Stock", callback_data="add_stock_start")]]
    for c in cats:
        ...
        buttons.append([InlineKeyboardButton(
            ...
            callback_data=f"stock_cat_{c['id']}")])
...
async def stock_cat_detail(update, context):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("✏️ Set Price",        callback_data=f"setprice_cat_{cat_id}"),
         InlineKeyboardButton("🔛 Toggle",            callback_data=f"toggle_cat_{cat_id}")],
        [InlineKeyboardButton("➕ Add More Numbers",  callback_data=f"addmore_cat_{cat_id}"),
         InlineKeyboardButton("🗑️ Delete Category",  callback_data=f"del_cat_{cat_id}")],
...
async def toggle_cat(update, context):
    # FIX: query.data can't be set — directly call stock_cat_detail logic
    query = update.callback_query
    await query.answer()
    if not is_admin(query.from_user.id):
        return
    ...
    # Directly show detail without setting query.data
    c = get_cat(cat_id)
    stock = get_stock_count(cat_id)
    conn2 = get_db()
    total = conn2.execute("SELECT COUNT(*) as cnt FROM accounts WHERE category_id=?",
...
async def del_cat(update, context):
    # FIX: query.data can't be set — directly call admin_stock logic
    query = update.callback_query
    await query.answer()
    if not is_admin(query.from_user.id):
        return
...
async def confirm_del(update, context):
    ...
    await query.edit_message_text("✅ Account deleted.",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Stock", callback_data="admin_stock")]]))
...
async def admin_orders(update, context):
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("◀️", callback_data=f"admin_orders_{sf}_{page-1}"))
        nav.append(InlineKeyboardButton(f"{page+1}/{pages}", callback_data="noop"))
    if page < pages - 1:
        nav.append(InlineKeyboardButton("▶️", callback_data=f"admin_orders_{sf}_{page+1}"))
    if nav:
...
async def admin_users(update, context):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔍 Search",       callback_data="admin_search_user"),
         InlineKeyboardButton("🚫 Ban",           callback_data="admin_ban_user"),
         InlineKeyboardButton("✅ Unban",          callback_data="admin_unban_user")],
        [InlineKeyboardButton("💰 Edit Wallet",   callback_data="admin_edit_wallet")],
...
async def admin_deps(update, context):
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("◀️", callback_data=f"admin_deps_{sf}_{page-1}"))
        nav.append(InlineKeyboardButton(f"{page+1}/{pages}", callback_data="noop"))
    if page < pages - 1:
        nav.append(InlineKeyboardButton("▶️", callback_data=f"admin_deps_{sf}_{page+1}"))
    if nav:
...
async def admin_settings(update, context):
    ...
    rate   = get_usdt_rate()
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton(f"🔧 Maintenance: {'ON→OFF' if maint else 'OFF→ON'}", callback_data="toggle_maintenance")],
        [InlineKeyboardButton(f"💳 UPI: {'✅ ON→Disable' if upi else '❌ OFF→Enable'}", callback_data="toggle_upi")],
        [InlineKeyboardButton(f"🪙 Crypto: {'✅ ON→Disable' if crypto else '❌ OFF→Enable'}", callback_data="toggle_crypto")],
        [InlineKeyboardButton(f"💱 USDT Rate: ₹{rate:.0f} → Change", callback_data="set_usdt_rate")],
